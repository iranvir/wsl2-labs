# WSL 2 Labs
> [!WARNING] 
> 1. `user.sh` has `rm -rf ` in it. If it runs amok say good bye to your C:\ D:\ 
> E:\ drives and all the data in there. Yes, it will totally nuke your Windows
> Host (speaking from experience). You have been warned. If unsure, remove lines 3,4,5,6 bit from `.\user.sh`.
...
4. Run `.\setup.ps1` with flags as described below:
    ...
    `.\setup.ps1 -Name A_NAME_FOR_YOUR_DISTRO -Rootfs PATH_TO_ROOTFS -UserName USER_NAME -EnableSystemd $false -IsAlpine $true`
